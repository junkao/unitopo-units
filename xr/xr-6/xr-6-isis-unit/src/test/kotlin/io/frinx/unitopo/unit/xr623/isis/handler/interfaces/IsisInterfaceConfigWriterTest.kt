...
import org.opendaylight.yang.gen.v1.http.cisco.com.ns.yang.cisco.ios.xr.clns.isis.cfg.rev151109.IsisConfigurableLevels as XrIsisConfigurableLevels
...
class IsisInterfaceConfigWriterTest {
    @Mock
    private lateinit var writeContext: WriteContext
    private lateinit var underlayAccess: UnderlayAccess
    private lateinit var target: IsisInterfaceConfigWriter

    private lateinit var idCap: ArgumentCaptor<IID<DataObject>>
    private lateinit var dataCap: ArgumentCaptor<XrInterface>
    ...
    companion object {
        ...
        private val IID_CONFIG = IidUtils.createIid(IIDs.NE_NE_PR_PR_IS_IN_IN_CONFIG,
    ...
    fun setUp() {
        ...
        target = IsisInterfaceConfigWriter(underlayAccess)

        idCap = ArgumentCaptor.forClass(IID::class.java) as ArgumentCaptor<IID<DataObject>>
        dataCap = ArgumentCaptor.forClass(XrInterface::class.java)
    }
    ...
    fun testWriteCurrentAttributesForType() {
        ...
        // from OpenConfig
        Assert.assertThat(dataCap.allValues[0].interfaceName.value, CoreMatchers.equalTo(INTERFACE_ID))
        Assert.assertThat(dataCap.allValues[0].circuitType, CoreMatchers.equalTo(XrIsisConfigurableLevels.Level1))
        Assert.assertThat(dataCap.allValues[0].isPointToPoint, CoreMatchers.`is`(true))
        Assert.assertThat(dataCap.allValues[0].isRunning, CoreMatchers.`is`(true))

        // from existing data
        Assert.assertThat(dataCap.allValues[0].lspRetransmitIntervals.lspRetransmitInterval!!.sortedBy { it.level },
                CoreMatchers.equalTo(listOf(
                        LspRetransmitIntervalBuilder().setLevel(IsisInternalLevel.NotSet).setInterval(100).build(),
                        LspRetransmitIntervalBuilder().setLevel(IsisInternalLevel.Level1).setInterval(200).build(),
                        LspRetransmitIntervalBuilder().setLevel(IsisInternalLevel.Level2).setInterval(300).build())))
        Assert.assertThat(dataCap.allValues[0].interfaceAfs.interfaceAf!![0].afName,
                CoreMatchers.equalTo(IsisAddressFamily.Ipv6))
        Assert.assertThat(dataCap.allValues[0].interfaceAfs.interfaceAf!![0].safName,
