...
class InterfaceReaderTest : AbstractNetconfHandlerTest() {
    ...
    private lateinit var ifaceReader: InterfaceReader

    @Before
    fun setUp() {
        MockitoAnnotations.initMocks(this)
        ifaceReader = InterfaceReader(underlayAccess)
    }
    ...
    fun testAllIds() {
        Assert.assertEquals(
                listOf("Null0", "GigabitEthernet0/0/0/0", "GigabitEthernet0/0/0/1", "GigabitEthernet0/0/0/2",
                        ...
                        .map { InterfaceKey(it) }
                        .toSet(),
                ifaceReader.parseInterfaceIds(parseGetCfgResponse(DATA_NODES, InterfaceReader.DATA_NODES_ID))
                        .toSet())
    }
}