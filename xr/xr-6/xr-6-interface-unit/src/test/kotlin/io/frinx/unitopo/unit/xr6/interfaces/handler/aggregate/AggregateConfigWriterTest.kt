...
class AggregateConfigWriterTest {
    ...
    companion object {
        ...
        private val NATIVE_IID_INTERFACE = InterfaceReader.IFC_CFGS
                .child(InterfaceConfiguration::class.java, InterfaceConfigurationKey(NATIVE_ACT, NATIVE_IF_NAME))
    ...
    fun testWriteCurrentAttributes() {
        ...
        Assert.assertThat(
                idCap.allValues[0],
                CoreMatchers.equalTo(NATIVE_IID_INTERFACE) as Matcher<in InstanceIdentifier<DataObject>>
        )

        Assert.assertThat(
                (dataCap.allValues[0] as InterfaceConfiguration).getAugmentation(MinLinksInterfaceConfig::class.java),
                CoreMatchers.equalTo(expectedConfigMinLinks) as Matcher<in DataObject>
        )
    }
