...
//        return keys
        ...
        val subIfcName = when (ifcIndex) {
            Util.ZERO_SUBINTERFACE_ID -> name
            else -> Util.getSubIfcName(name, ifcIndex)
        }
        val keys = mutableListOf<AddressKey>()
    ...
    companion object {
        fun extractAddresses(ifcCfg: InterfaceConfiguration, keys: MutableList<AddressKey>) {
            ifcCfg.getAugmentation(Ipv4IfcAugment::class.java)?.let { interfaceConfiguration1 ->
                interfaceConfiguration1.ipv4Network?.let { ipv4Network ->
                    ipv4Network.addresses?.let { addresses ->
                        addresses.primary?.let {
