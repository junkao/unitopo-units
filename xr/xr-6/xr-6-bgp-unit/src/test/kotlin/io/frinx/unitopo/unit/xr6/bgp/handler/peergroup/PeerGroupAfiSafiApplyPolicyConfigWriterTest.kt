...
import org.opendaylight.yang.gen.v1.http.cisco.com.ns.yang.cisco.ios.xr.ipv4.bgp.cfg.rev150827.bgp.instance.instance.`as`.four._byte.`as`._default.vrf.bgp.entity.neighbor.groups.neighbor.group.NeighborGroupAfs
import org.opendaylight.yang.gen.v1.http.cisco.com.ns.yang.cisco.ios.xr.ipv4.bgp.cfg.rev150827.bgp.instance.instance.`as`.four._byte.`as`._default.vrf.bgp.entity.neighbor.groups.neighbor.group.neighbor.group.afs.NeighborGroupAf
...
import org.opendaylight.yang.gen.v1.http.cisco.com.ns.yang.cisco.ios.xr.ipv4.bgp.cfg.rev150827.bgp.instance.instance.`as`.four._byte.`as`._default.vrf.bgp.entity.neighbor.groups.neighbor.group.neighbor.group.afs.NeighborGroupAfKey
...
class PeerGroupAfiSafiApplyPolicyConfigWriterTest : AbstractNetconfHandlerTest() {
    ...
    private lateinit var target: PeerGroupAfiSafiApplyPolicyConfigWriter
    ...
    fun setUp() {
        ...
        target = PeerGroupAfiSafiApplyPolicyConfigWriter(underlayAccess!!)
    }

    @Test
    fun testWriteCurrentAttributes() {
        val config = ConfigBuilder()
            .setImportPolicy(listOf("POLICY_IN"))
            .setExportPolicy(listOf("POLICY_OUT"))
            .build()
        ...
        Assert.assertEquals(idCap.allValues.get(0), NATIVE_IID)
    }
    ...
    fun testdeleteCurrentAttributes() {
        ...
        Mockito.verify(underlayAccess!!,
            Mockito.times(1)).safeMerge(Mockito.any(), Mockito.any(), idCap.capture(), dataCap.capture())
        Assert.assertEquals(idCap.allValues.size, 1)
        Assert.assertEquals(idCap.allValues.get(0), NATIVE_IID)
    }
}