...
import org.opendaylight.yang.gen.v1.http.cisco.com.ns.yang.cisco.ios.xr.ipv4.bgp.cfg.rev180615.bgp.instance.instance.`as`.four._byte.`as`._default.vrf.bgp.entity.neighbor.groups.NeighborGroupBuilder
...
class PeerGroupConfigWriterTest : AbstractNetconfHandlerTest() {
    ...
    private lateinit var target: PeerGroupConfigWriter

    companion object {
        private val IID_CONFIG = PeerGroupListReaderTest.id.child(Config::class.java)
    ...
    fun setUp() {
        ...
        target = PeerGroupConfigWriter(underlayAccess!!)
    }

    @Test
    fun testWriteCurrentAttributes() {
        val config = ConfigBuilder()
            .setPeerGroupName(PeerGroupListReaderTest.PEER_GROUP_NAME1)
            .build()
        ...
        Mockito.`when`(future.checkedGet()).thenReturn(Optional.of(NeighborGroupBuilder().build()))
