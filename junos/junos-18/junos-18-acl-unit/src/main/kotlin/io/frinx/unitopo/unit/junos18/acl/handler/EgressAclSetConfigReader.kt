...
class EgressAclSetConfigReader : ConfigReaderCustomizer<Config, ConfigBuilder> {

    override fun getBuilder(id: IID<Config>): ConfigBuilder = ConfigBuilder()

    override fun merge(builder: Builder<out DataObject>, config: Config) {
        (builder as EgressAclSetBuilder).config = config
    }

    override fun readCurrentAttributes(id: IID<Config>, config: ConfigBuilder, readContext: ReadContext) {
        val key = id.firstKeyOf(EgressAclSet::class.java)

        config.setName = key.setName
